...
5. **Dependency, CI/CD, module-doc, and agent-guide cleanup**
   ...
   Current dependency/CI/CD status: the first v3.5 infra slice refreshes
   lockfile entries that fit existing semver ranges, adds clean-checkout CI,
   runs backend API tests in CI against disposable SQLite fixtures, and adds a
   manual deploy workflow around the tracked remote scripts. Major or ecosystem
   upgrades remain deferred into focused branches: React Router 8, Vite 8,
   i18next 26, i18next-http-backend 4, react-i18next 17, lucide-react 1,
   shadcn 4, vite-tsconfig-paths 6, @types/node 26, and the Prisma dev-chain
   audit fix once a Prisma 7-compatible path is available.
