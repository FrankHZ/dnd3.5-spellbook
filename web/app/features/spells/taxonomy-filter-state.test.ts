...
describe("taxonomy filter state helpers", () => {
  it("parses positive taxonomy id lists from URL params", () => {
    expect(
      parseTaxonomyFilters(
        new URLSearchParams(
          "schoolIds=2,1,1&subschoolIds=0,4&descriptorIds=bad,6",
        ),
      ),
    ).toEqual({
      schoolIds: [1, 2],
  ...
  it("sets and deletes taxonomy URL params", () => {
    const params = new URLSearchParams(
      "schoolIds=1&subschoolIds=2&descriptorIds=3&page=4",
    );
    ...
    expect(String(params)).toBe("schoolIds=5&descriptorIds=8%2C9&page=4");
  });
  ...
  it("counts active taxonomy filters", () => {
    ...
    expect(countTaxonomyFilters(filters)).toBe(3);
  });
});
